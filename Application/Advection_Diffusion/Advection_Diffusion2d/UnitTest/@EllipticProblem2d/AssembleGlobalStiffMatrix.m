...
OP11 = -1 * (ElementMassMatrix\(obj.D21*Dx'*ElementMassMatrix*Dy) ) -1 * ( ElementMassMatrix\(obj.D12*Dy'*ElementMassMatrix*Dx) )...
    -1 * (ElementMassMatrix\(obj.D11 * Dx' * ElementMassMatrix * Dx + obj.D22 * Dy' * ElementMassMatrix * Dy));  %checked
...
Tau = bsxfun(@times,  max(obj.D11,max(obj.D12,max(obj.D21,obj.D22))) * (fm(:) )',...
...
for face = 1:InnerEdge.Ne
    ...
    OP11(:, eidM) = OP11(:, eidM) + 0.5*obj.D21*Dx(eidM,:)'*diag(ny)*FacialMass2d+ 0.5*obj.D12*Dy(eidM,:)'*diag(nx)*FacialMass2d + 0.5 * obj.D11 * ( Dx(eidM,:) )' * ( FacialMass2d ) * diag(nx) + 0.5 * obj.D22 * ( Dy(eidM,:) )' * ( FacialMass2d ) * diag(ny);   %checked
    OP12(:, eidM) = OP12(:, eidM) + 0.5*obj.D21*Dx(eidP,:)'*diag(ny)*FacialMass2d+ 0.5*obj.D12*Dy(eidP,:)'*diag(nx)*FacialMass2d + 0.5 * obj.D11 * ( Dx(eidP,:) )' * ( FacialMass2d ) * diag(nx) + 0.5 * obj.D22 * ( Dy(eidP,:) )' * ( FacialMass2d ) * diag(ny);   %checked
    OP22(:, eidP) = OP22(:, eidP) + 0.5*obj.D21*Dx(eidP,:)'*diag(-1*ny)*FacialMass2d+ 0.5*obj.D12*Dy(eidP,:)'*diag(-1*nx)*FacialMass2d + 0.5 * obj.D11 * ( Dx(eidP,:) )' * ( FacialMass2d ) * diag(-1*nx) + 0.5 * obj.D22 * ( Dy(eidP,:) )' * ( FacialMass2d ) * diag(-1*ny);
    OP21(:, eidP) = OP21(:, eidP) + 0.5*obj.D21*Dx(eidM,:)'*diag(-1*ny)*FacialMass2d+ 0.5*obj.D12*Dy(eidM,:)'*diag(-1*nx)*FacialMass2d + 0.5 * obj.D11 * ( Dx(eidM,:) )' * ( FacialMass2d ) * diag(-1*nx) + 0.5 * obj.D22 * ( Dy(eidM,:) )' * ( FacialMass2d ) * diag(-1*ny);
    %> For term $\left {\nabla_y q_h\right }[v]_x$
    OP11(eidM, :) = OP11(eidM, :) + 0.5 * obj.D12 * diag(nx)*FacialMass2d*Dy(eidM,:)+ 0.5 * obj.D21 * diag(ny)*FacialMass2d*Dx(eidM,:) + 0.5 * obj.D11 * FacialMass2d * diag(nx) * Dx(eidM,:) + 0.5 * obj.D22 * FacialMass2d * diag(ny) * Dy(eidM,:);
    OP12(eidP, :) = OP12(eidP, :) + 0.5 * obj.D12 * diag(-1*nx)*FacialMass2d*Dy(eidM,:)+ 0.5 * obj.D21 * diag(-1*ny)*FacialMass2d*Dx(eidM,:) + 0.5 * obj.D11 * FacialMass2d * diag(-1*nx) * Dx(eidM,:) + 0.5 * obj.D22 * FacialMass2d * diag(-1*ny) * Dy(eidM,:);
    OP22(eidP, :) = OP22(eidP, :) + 0.5 * obj.D12 * diag(-1*nx)*FacialMass2d*Dy(eidP,:)+ 0.5 * obj.D21 * diag(-1*ny)*FacialMass2d*Dx(eidP,:) + 0.5 * obj.D11 * FacialMass2d * diag(-1*nx) * Dx(eidP,:) + 0.5 * obj.D22 * FacialMass2d * diag(-1*ny) * Dy(eidP,:);
    OP21(eidM, :) = OP21(eidM, :) + 0.5 * obj.D12 * diag(nx)*FacialMass2d*Dy(eidP,:)+ 0.5 * obj.D21 * diag(ny)*FacialMass2d*Dx(eidP,:) + 0.5 * obj.D11 * FacialMass2d * diag(nx) * Dx(eidP,:) + 0.5 * obj.D22 * FacialMass2d * diag(ny) * Dy(eidP,:);
...
for face = 1:BoundaryEdge.Ne
    ...
    switch(BoundaryEdge.ftype(face))
        ...
        case enumBoundaryCondition.Dirichlet
            ...
            OP11(eidM,:) = OP11(eidM,:) + obj.D12 * diag(nx)*FacialMass2d*Dy(eidM,:)+ obj.D21 * diag(ny)*FacialMass2d*Dx(eidM,:) + obj.D11 * FacialMass2d * diag(nx) * Dx(eidM,:) + obj.D22 * FacialMass2d * diag(ny) * Dy(eidM,:);
