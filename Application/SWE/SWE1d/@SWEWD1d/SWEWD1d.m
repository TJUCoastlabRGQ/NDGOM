...
%                 temp(:, ind2) = 0;
                ...
                flg = (mesh.EToR == int8( enumSWERegion.PartialWetDamBreak ) ) ...
                    | (mesh.EToR == int8( enumSWERegion.PartialWetFlood ));
...
%                 end
        ...
        function matUpdateWetDryState(obj, fphys)
            for m = 1:obj.Nmesh
                ...
                mesh.status( wetflg ) = int8( enumSWERegion.Wet );
                % dry part
                dryflag = all( fphys{m}(:,:,1) < obj.hmin );
                mesh.status( dryflag ) = int8( enumSWERegion.Dry );
                % partial wet part
                ...
                pdblg = dblg & pwlg;
                pflg = (~dblg) & pwlg;
                mesh.status( pdblg ) = int8( enumSWERegion.PartialWetDamBreak );
                mesh.status( pflg ) = int8( enumSWERegion.PartialWetFlood );
