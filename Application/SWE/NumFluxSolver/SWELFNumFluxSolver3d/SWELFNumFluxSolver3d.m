classdef SWELFNumFluxSolver3d
    ...
    methods( Access = protected )
        ...
        function [ FluxS ] = LFNumFluxTerm( obj, physClass, hmin, gra, fm, fp, nx, ny, nz )
            
            FluxM(:, :, 1) = ( fm(:,:,1).^2./fm(:,:,4)  + 1/2 * gra * ( fm(:,:,4).^2 - physClass.H0.^2 ) ) .* nx +...
                ( fm(:,:,1) .* fm(:,:,2) ./ fm(:,:,4) ) .* ny + ( fm(:,:,1) .* fm(:,:,3) ./ fm(:,:,4) ) .* nz;
            %> $(hu^2+\frac{1}{2}g(H^2 - h^2) )* nx + huv * ny + u\omega * nz$
            FluxP(:, :, 1) = ( fp(:,:,1).^2./fp(:,:,4) + 1/2 * gra * ( fp(:,:,4).^2 - physClass.H0.^2 )) .* nx +...
                ( fp(:,:,1) .* fp(:,:,2) ./ fp(:,:,4) ) .* ny+ ( fp(:,:,1) .* fp(:,:,3) ./ fp(:,:,4) ) .* nz;
            %> $huv * nx + ( hv^2+\frac{1}{2}g(H^2 - h^2) ) * ny + v\omega * nz$
            FluxM(:, :, 2) = ( fm(:,:,1) .* fm(:,:,2) ./ fm(:,:,4) ) .* nx +...
                ( fm(:,:,2).^2./fm(:,:,4) + 1/2 * gra * ( fm(:,:,4).^2 - physClass.H0.^2 ) ) .* ny +...
                 ( fm(:,:,2) .* fm(:,:,3) ./ fm(:,:,4) ) .* nz;
            %> $huv * nx + ( hv^2+\frac{1}{2}g(H^2 - h^2) ) * ny + v\omega * nz$
            FluxP(:, :, 2) = ( fp(:,:,1) .* fp(:,:,2) ./ fp(:,:,4) ) .* nx +...
                ( fp(:,:,2).^2./fp(:,:,4) + 1/2 * gra * ( fp(:,:,4).^2 - physClass.H0.^2 ) ) .* ny+ ...
                ( fp(:,:,2) .* fp(:,:,3) ./ fp(:,:,4) ) .* nz;


%             FluxM(:, :, 1) = ( fm(:,:,1).^2./fm(:,:,4)  + 1/2 * gra * ( fm(:,:,4).^2 ) ) .* nx +...
%                 ( fm(:,:,1) .* fm(:,:,2) ./ fm(:,:,4) ) .* ny + ( fm(:,:,1) .* fm(:,:,3) ./ fm(:,:,4) ) .* nz;
%             %> $(hu^2+\frac{1}{2}g(H^2) )* nx + huv * ny + u\omega * nz$
%             FluxP(:, :, 1) = ( fp(:,:,1).^2./fp(:,:,4) + 1/2 * gra * ( fp(:,:,4).^2)) .* nx +...
%                 ( fp(:,:,1) .* fp(:,:,2) ./ fp(:,:,4) ) .* ny+ ( fp(:,:,1) .* fp(:,:,3) ./ fp(:,:,4) ) .* nz;
%             %> $huv * nx + ( hv^2+\frac{1}{2}g(H^2) ) * ny + v\omega * nz$
%             FluxM(:, :, 2) = ( fm(:,:,1) .* fm(:,:,2) ./ fm(:,:,4) ) .* nx +...
%                 ( fm(:,:,2).^2./fm(:,:,4) + 1/2 * gra * ( fm(:,:,4).^2 ) ) .* ny +...
%                  ( fm(:,:,2) .* fm(:,:,3) ./ fm(:,:,4) ) .* nz;
%             %> $huv * nx + ( hv^2+\frac{1}{2}g(H^2) ) * ny + v\omega * nz$
%             FluxP(:, :, 2) = ( fp(:,:,1) .* fp(:,:,2) ./ fp(:,:,4) ) .* nx +...
%                 ( fp(:,:,2).^2./fp(:,:,4) + 1/2 * gra * ( fp(:,:,4).^2 ) ) .* ny+ ...
%                 ( fp(:,:,2) .* fp(:,:,3) ./ fp(:,:,4) ) .* nz;
            ...
            lambda = abs( max( vertcat (max ( max ( fm(:, :, 1) ./ fm(:, :, 4) .* nx +  fm(:, :, 2) ./ fm(:, :, 4) .* ny + fm(:,:,3) ./ fm(:,:,4) .* nz,...
                ...
                max( max (( fm(:, :, 1) ./ fm(:, :, 4) + sqrt(gra* fm(:, :, 4))) .* nx...
                + ( fm(:, :, 2) ./ fm(:, :, 4) + sqrt(gra* fm(:, :, 4))) .* ny + fm(:,:,3) ./ fm(:,:,4) .* nz ,...
                ( -fp(:, :, 1) ./ fp(:, :, 4) - sqrt(gra* fp(:, :, 4))) .* nx...
                + ( -fp(:, :, 2) ./ fp(:, :, 4) - sqrt(gra* fp(:, :, 4))) .* ny - fp(:,:,3) ./ fp(:,:,4) .* nz )),...
                max( max (( fm(:, :, 1) ./ fm(:, :, 4) - sqrt(gra* fm(:, :, 4))) .* nx...
                + ( fm(:, :, 2) ./ fm(:, :, 4) - sqrt(gra* fm(:, :, 4))) .* ny + fm(:,:,3) ./ fm(:,:,4) .* nz,...
                ( -fp(:, :, 1) ./ fp(:, :, 4) +  sqrt(gra* fp(:, :, 4))) .* nx...
                + ( -fp(:, :, 2) ./ fp(:, :, 4) + sqrt(gra* fp(:, :, 4))) .* ny - fp(:,:,3) ./ fp(:,:,4) .* nz )))));
