...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    double *hwm = mxGetPr(prhs[5]);
    double *hp = mxGetPr(prhs[6]);
    double *hup = mxGetPr(prhs[7]);
    double *hvp = mxGetPr(prhs[8]);
    double *hwp = mxGetPr(prhs[9]);
        ...
        if (DryFaceFlag[k] == 1) {
            ...
            for(mwIndex i = 0; i < Nfp; i++){
                ...
                if(hum[j] * nx[j] + hvm[j] * ny[j] > 0 && -hup[j] * nx[j] - hvp[j] * ny[j] <= 0)
                    fluxS[j] = hwm[j] * ( hum[j] * nx[j] + hvm[j] * ny[j] )/hm[j];
                else if(hum[j] * nx[j] + hvm[j] * ny[j] <= 0 && -hup[j] * nx[j] - hvp[j] * ny[j] > 0)
                    fluxS[j] = hwp[j] * ( hup[j] * nx[j] + hvp[j] * ny[j] )/hp[j];
                else
                    fluxS[j] = ( hwm[j] * hum[j]/hm[j] + hwp[j] * hup[j]/hp[j] ) * nx[j] / 2 + ( hwm[j] * hvm[j]/hm[j] + hwp[j] * hvp[j]/hp[j] ) * ny[j] / 2;
            }
        }
    }
