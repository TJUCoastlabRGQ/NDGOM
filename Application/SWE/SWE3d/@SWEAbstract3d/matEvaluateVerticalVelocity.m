function [ Omega , W ] = matEvaluateVerticalVelocity( obj, mesh3d, fphys2d, fphys3d )
...
%                 [ InnerSurface_frhs2d ] = edge2d.matEvaluateStrongFromEdgeRHS( InnerEdgeFluxM2d{1}(:,:,1), InnerEdgeFluxP2d{1}(:,:,1), InnerEdgeFluxS2d{1}(:,:,1) );
% 
...
%                 [ BoundarySurface_frhs2d ] = edge2d.matEvaluateStrongFromEdgeRHS( BoundaryFluxM2d{1}(:,:,1), BoundaryEdgeFluxS2d{1}(:,:,1) );
...
W = Omega + ( 1 + mesh3d.z ) .* ( - ( dHUHV ) - mesh3d.tz .* (mesh3d.cell.Dt * Omega) ) + ...
    fphys3d{1}(:,:,1)./fphys3d{1}(:,:,4) .* (1 + mesh3d.z) .* ( mesh3d.rx .* (mesh3d.cell.Dr * fphys3d{1}(:,:,4))...
    + mesh3d.sx .* (mesh3d.cell.Ds * fphys3d{1}(:,:,4)) ) + fphys3d{1}(:,:,2)./fphys3d{1}(:,:,4) .* (1 + mesh3d.z)...
    .* ( mesh3d.ry .* (mesh3d.cell.Dr * fphys3d{1}(:,:,4)) + mesh3d.sy .* (mesh3d.cell.Ds * fphys3d{1}(:,:,4)) );
end
