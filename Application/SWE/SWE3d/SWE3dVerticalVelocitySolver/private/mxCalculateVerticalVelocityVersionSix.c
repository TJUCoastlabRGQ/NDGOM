...
#pragma omp parallel for num_threads(DG_THREADS)
	...
	/*Jz in three-dimensional mesh*/
	mxArray *TempJz = mxGetField(prhs[1], 0, "Jz");
	double *Jz = mxGetPr(TempJz);
	/*Data contained in three-dimensional cell*/
	...
	double *fmod = malloc(K2d*Np3d * sizeof(double));

#ifdef _OPENMP
#pragma omp parallel for num_threads(DG_THREADS)
#endif
	for (int k = 0; k < K2d; k++) {
		VerticalIntegralFromBottom(VerticalVelocity + k*NLayer*Np3d, VSrhs3d + k*NLayer*Np3d, Jz + k*NLayer*Np3d, fmod + k*Np3d, NLayer, (ptrdiff_t)Np3d, InvV3d, Np2d, Npz, Vint);
	}
