...
s = sech(alpha*(x*h-150));
t = tanh(alpha*(x*h-150));
eta = h*(1 + epsilon * s^2 - 3/4*epsilon^2*s^2*t^2 + epsilon^3*(5/8*s^2*t^2-101/80*s^4*t^2));
y = eta * (1+z);
u = (1 - 1 + 1/2*epsilon - 3/20*epsilon^2 + 3/56*epsilon^3 - epsilon*s^2 + epsilon^2*(-1/4*s^2+s^4+y^2*(3/2*s^2-9/4*s^4)) + ...
    epsilon^3*(19/40*s^2 + 1/5*s^4-6/5*s^6 + y^2*(-3/2*s^2-15/4*s^4+15/2*s^6) + y^4*(-3/8*s^2+45/16*s^4-45/16*s^6)) )*sqrt(obj.gra*h);
w = sqrt(3*epsilon)*y*t*(-1*epsilon*s^2+epsilon^2*(3/8*s^2+2*s^4+y^2*(1/2*s^2-3/2*s^4)) + ...
    epsilon^3*(49/640*s^2 - 17/20*s^4 - 18/5*s^6 + y^2*(-13/16*s^2-25/16*s^4+15/2*s^6)+y^4*(-3/40*s^2+9/8*s^4-27/16*s^6)))*sqrt(obj.gra*h);
...
% t = 20;
x = mesh.mesh2d.x + 20*sqrt(obj.gra*h*(1+epsilon));
obj.H20 = eval(eta);
z = -0.5;
obj.CEU20 = eval(u);
obj.CEP20 = eval(p);
...
% t = 30;
x = mesh.mesh2d.x + 30*sqrt(obj.gra*h*(1+epsilon));
obj.H30 = eval(eta);
z = -0.5;
obj.CEU30 = eval(u);
obj.CEP30 = eval(p);
