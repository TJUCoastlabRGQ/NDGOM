...
for n = 1:Ndeg
    for m = 1:Nmesh
        Solver = ConstAdvectionNoSource3d(Order(n), Order(n), M(m), M(m));
        tic;
        Solver.matSolve;
        time(m,n) = toc;
        len(m, n) = 2/Solver.M;
        dofs(m,n) = numel(Solver.fphys{1}(:,:,1));
        PostProcess = NdgPostProcess(Solver.meshUnion(1),strcat('ConstAdvectionNoSource3d/3d','/','ConstAdvectionNoSource3d'));
        fext = cell(1);
