...
for n = 1:Ndeg
    for m = 1:Nmesh
        ...
        fphys = cell(1);
        fphys{1}(:,:,1:4) = Solver.fphys{1}(:,:,[4 1 2 3]);
        err = PostProcess.evaluateNormErrInf( fphys, ExactValue );
        HErrInf( m, n ) = err(1); HUErrInf( m, n ) = err(2); HVErrInf( m, n ) = err(3); OmegaErrInf( m, n ) = err(4);
        
        err = PostProcess.evaluateNormErr2( fphys, ExactValue );
        HErr2( m, n ) = err(1); HUErr2( m, n ) = err(2); HVErr2( m, n ) = err(3); OmegaErr2( m, n ) = err(4);
        
        err = PostProcess.evaluateNormErr1( fphys, ExactValue );
        HErr1( m, n ) = err(1); HUErr1( m, n ) = err(2); HVErr1( m, n ) = err(3);OmegaErr1( m, n ) = err(4);
    ...
    plot(dofs(:, n), OmegaErr1(:, n), [co, marker{4}, linestyle],...
        'LineWidth', linewidth, ...
        'MarkerSize', markersize ...
        );    
    ...
    plot(dofs(:, n), OmegaErr2(:, n), [co, marker{4}, linestyle],...
        'LineWidth', linewidth, ...
        'MarkerSize', markersize ...
        );    
    ...
    plot(dofs(:, n), OmegaErrInf(:, n), [co, marker{4}, linestyle],...
        'LineWidth', linewidth, ...
        'MarkerSize', markersize ...
        );  
