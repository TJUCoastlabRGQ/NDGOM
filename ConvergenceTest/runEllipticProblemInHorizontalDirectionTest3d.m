...
for n = 1:Ndeg
    for m = 1:Nmesh
        Solver = EllipticProblemInHorizontalDirectionTest3d(Order(n), Order(n), M(m), Mz(m));
        Solver.EllipticProblemSolve;
        len(m, n) = M(m);
        dofs(m,n) = numel(Solver.meshUnion(1).x);
        PostProcess = NdgPostProcess(Solver.meshUnion(1),strcat('EllipticProblemInHorizontalDirectionTest3d/3d','/','EllipticProblemInHorizontalDirectionTest3d'));
        ExactValue = cell(1);
