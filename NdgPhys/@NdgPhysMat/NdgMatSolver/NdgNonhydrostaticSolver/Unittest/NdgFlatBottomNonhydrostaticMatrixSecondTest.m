classdef NdgFlatBottomNonhydrostaticMatrixSecondTest < NdgFlatBottomWetDryTestWithWallBoundary
    ...
    methods
        function obj = NdgFlatBottomNonhydrostaticMatrixSecondTest(N, cellType)
            obj = obj@NdgFlatBottomWetDryTestWithWallBoundary(N, cellType);
        end
        
        function matrixX = getExactMatrixX(obj)
            matrixX = zeros(size(obj.meshUnion(1).x));
        end
        
        function matrixY = getExactMatrixY(obj)
            matrixY = ones(size(obj.meshUnion(1).x));
        end
        ...
        function ExactStiffMatrix = getGlobalStiffMatrix(obj)
            ...
            for i = 1:K*Np
                ...
                ExactStiffMatrix(:,i) = ExactStiffMatrix(:,i) + 1/obj.rho .* Exactqy(:) - 1/obj.rho .* height(:).*Exactq2x(:)-...
