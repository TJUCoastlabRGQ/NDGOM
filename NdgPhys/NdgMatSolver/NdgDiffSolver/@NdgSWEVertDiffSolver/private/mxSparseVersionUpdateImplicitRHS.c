...
extern int *Ir, *Jc, *QuatrIr, *QuatrJc;

extern int NNZ, QuatrNNZ;
...
void AssembleGlobalStiffMatrixForhuv(double *dest, double *Finaldest, double *source, double *Finalsource,\
	double *Coriolis, double dt, double ImplicitParam, int Nz, int Np) {
	int Num;
	int Dstart;
	int Sstart;
...
#pragma omp parallel for num_threads(DG_THREADS) private(var,j) if(K2d>50)
    ...
    for (i = 0; i < K2d; i++){
		...
		/*Stiff matrix corresponds to hu and hv, they are placed into one, as coriolis term treated implicitly*/
		double *StiffMatrixForhuv = malloc(QuatrNNZ * sizeof(double));
		memset(StiffMatrixForhuv, 0, QuatrNNZ * sizeof(double));
		...
		/*Final stiff matrix corresponds to hu and hv, they are placed into one, as coriolis term treated implicitly*/
		double *FinalStiffMatrixForhuv = malloc(QuatrNNZ * sizeof(double));
		memset(FinalStiffMatrixForhuv, 0, QuatrNNZ * sizeof(double));
		/*Continious space in memory for hu and hv*/
		double *Temphuv = malloc(Nz*Np * 2 * sizeof(double));
		/*Continious space in memory for the global right hand side*/
		double *TempGlobalRHS = malloc(Nz*Np * 2 * sizeof(double));
		/*Continious space in memory for the implicit right hand side*/
		double *TempImplicitRHS = malloc(Nz*Np * 2 * sizeof(double));
		...
		free(FinalStiffMatrixForhuv);
		free(Temphuv);
		free(TempGlobalRHS);
		free(TempImplicitRHS);
        free(EleMass3d);
        free(InvEleMass3d);
        free(EleMass2d);
