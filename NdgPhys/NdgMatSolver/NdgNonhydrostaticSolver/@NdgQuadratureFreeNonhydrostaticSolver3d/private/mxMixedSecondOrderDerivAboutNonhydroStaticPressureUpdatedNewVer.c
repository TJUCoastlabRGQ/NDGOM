...
about nonhydrostatic pressure, i.e. $\frac{\partial}{\partial x}(\frac{\partial q}{\partial \sigma}) +
\frac{\partial}{\partial \sigma}(\frac{\partial q}{\partial x})$, $\frac{\partial}{\partial y}(\frac{\partial q}{\partial \sigma}) +
\frac{\partial}{\partial \sigma}(\frac{\partial q}{\partial y})$. The following primal form is given for the first term.
For this term, the primal form is given as:

$$-\int_{\Omega}\frac{\partial v}{\partial \sigma}\frac{\partial p}{\partial x}d\Omega -\\
...
\int_{\epsilon_D}\frac{\partial v}{\partial \sigma}n_x pd\Omega+ \int_{\epsilon_D}\frac{\partial v}{\partial x}n_{\sigma} pd\Omega  \\
+ \int_{\epsilon_i}\left \{\frac{\partial p}{\partial \sigma} \right \}[v]_xd\Omega+ \int_{\epsilon_i}\left \{\frac{\partial p}{\partial x} \right \}[v]_{\sigma}d\Omega\\
+ \int_{\epsilon_D}\frac{\partial p}{\partial \sigma}n_x vd\Omega+ \int_{\epsilon_D}\frac{\partial p}{\partial x}n_{\sigma} vd\Omega - \\
\int_{\epsilon_i}\tau[v]_x[p]_xd\Omega - \int_{\epsilon_i}\tau[v]_{\sigma}[p]_{\sigma}d\Omega\\
- \int_{\epsilon_D}n_x^2\tau vpd\Omega - \int_{\epsilon_D}n_{\sigma}^2\tau vpd\Omega = \\
\int_{\epsilon_D}\frac{\partial v}{\partial \sigma}n_x g_Dd\Omega+ \\
\int_{\epsilon_D}\frac{\partial v}{\partial x}n_{\sigma} g_Dd\Omega + \int_{\epsilon_D}\tau v g_Dd\Omega-\int_{\epsilon_N}vg_Nd\Omega$$*/
...
void GetLocalVolumuIntegralTermForMixedSecondOrderTerm(double *dest, int StartPoint, int Np, int NonzeroPerColumn, \
	...
	// For term $$ - \int_{ \Omega }\frac{ \partial v }{\partial x}\frac{ \partial p }{\partial \sigma}d\Omega$$
	...
	// For term $$ - \int_{ \Omega }\frac{ \partial v }{\partial \sigma}\frac{ \partial p }{\partial x}d\Omega$$
	MatrixMultiply("T", "N", (ptrdiff_t)Np, (ptrdiff_t)Np, (ptrdiff_t)Np, 1.0, VerticalDiffMatrix,
		(ptrdiff_t)Np, Mass3d, (ptrdiff_t)Np, 0.0, TempContributionBuff, (ptrdiff_t)Np);
	MatrixMultiply("N", "N", (ptrdiff_t)Np, (ptrdiff_t)Np, (ptrdiff_t)Np, 1.0, TempContributionBuff,
...
void GetLocalToAdjacentElementContributionInHorizontalDirection(double *dest, int LocalEle, int *AdjEle, double *FacialVector, int *GlobalFace,\
	...
	for (int i = 0; i < InternalFace; i++){
		...
		AssembleFacialDiffMatrix(FacialDiffMatrix, LocalDiff, AdjEidM, Nfp, Np);
...
/*The following part is used to assemble term corresponding to:
$$\int_{\epsilon_D}\frac{\partial v}{\partial \sigma}n_x pd\Omega+ \\
\int_{\epsilon_D}\frac{\partial v}{\partial x}n_{\sigma} pd\Omega + \\
\int_{\epsilon_D}\frac{\partial p}{\partial \sigma}n_x vd\Omega+ \\
\int_{\epsilon_D}\frac{\partial p}{\partial x}n_{\sigma} vd\Omega - \\
\int_{\epsilon_D}n_x^2\tau vpd\Omega - \int_{\epsilon_D}n_{\sigma}^2\tau vpd\Omega$$, 
here, the first, third and fifth terms, i.e. $\int_{\epsilon_D}\frac{\partial v}{\partial \sigma}n_x pd\Omega$,
$\int_{\epsilon_D}\frac{\partial p}{\partial \sigma}n_x vd\Omega$ and $\int_{\epsilon_D}n_x^2\tau vpd\Omega$ are
zero since $n_x = 0$ on the top boundary boundary.
The input parameter are as follows:
dest: A pointer to the start of the sparse matrix
...
\int_{\epsilon_i}\left \{\frac{\partial p}{\partial x} \right \}[v]_{\sigma}d\Omega  - \\
\int_{\epsilon_i}\tau[v]_x[p]_xd\Omega - \int_{\epsilon_i}\tau[v]_{\sigma}[p]_{\sigma}d\Omega$$.
For this primal form, the first, third and the fifth terms, i.e. $\int_{\epsilon_i}\left\{\frac{\partial v}{\partial \sigma}\right \}[p]_xd\Omega$,
$\int_{\epsilon_i}\left \{\frac{\partial p}{\partial \sigma} \right \}[v]_xd\Omega$ and $\int_{\epsilon_i}\tau[v]_x[p]_xd\Omega$
equal to zero since $n_x=0$ on the horizontal face.
The input parameter are as follows:
dest: A pointer to the start of the sparse matrix
Np: Number of interpolation point for the 3d element
Np2d: Number of interpolation point for the 2d element. For the current case, the cell on top of the 3d element.
