function termY = matCalculateUpwindedConservativeVariableRelatedMatrixY(obj, physClass, BoundaryEdge, InnerEdge, fphys, ftype, index)
...
fluxSY = BoundaryEdge.ny .* (1 + sign(BoundaryEdge.ny .* fm(:,:,index)))./2 .* fm(:,:,index) + BoundaryEdge.ny .*  (1 + sign( -BoundaryEdge.ny .* fp(:,:,index)))./2 .* fp(:,:,index);
