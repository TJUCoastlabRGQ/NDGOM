 ...
 *		double[1] 	Js the facial Jacobian of the studied face
 */
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    double *BoundaryEdge_FToE = mxGetPr(prhs[5]);
    double *InnerEdge_nx = mxGetPr(prhs[6]);
    double *InnerEdge_FToE = mxGetPr(prhs[7]);
    double *BoundaryEdge_Js = mxGetPr(prhs[8]);
    double *InnerEdge_Js = mxGetPr(prhs[9]);
    ...
    if ( ele == adjacentEle ){ /*the studied element is adjacent to the boundary edge*/
        for(mwIndex i = 0; i < BoundaryEdge_Ne; i++){
            if ( ele == BoundaryEdge_FToE[2*i] && face == BoundaryEdge_FToF[2*i] ){
                ...
                plhs[1] = mxCreateDoubleScalar(BoundaryEdge_Js[BoundaryEdge_Nfp*i]);
                break;
            }
        }
    } 
        ...
        for(mwIndex i = 0; i < InnerEdge_Ne; i++ ){
            ...
            if( ele == ele1 && adjacentEle == ele2 ){
              ...
              plhs[1] = mxCreateDoubleScalar(InnerEdge_Js[InnerEdge_Nfp*i]);
              break;
            }
